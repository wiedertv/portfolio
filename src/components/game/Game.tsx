...
export function Game({ paused }: { paused: boolean }) {
  ...
  useEffect(() => {
    ...
    const unsubscribe = events.on("ready", () => {
      if (disposed) return;
      setStatus("ready");
  ...
  return (
    <div
      ...
      tabIndex={paused ? -1 : 0}
      role="region"
      aria-label="Interactive village. Press WASD or arrow keys to move. Press Tab to leave the game."
